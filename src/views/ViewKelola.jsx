...
// ViewKelola — kelola menu, kategori, dan pengguna (CRUD).
function ViewKelola({
  menu, cats, allCats,                  // menuH
  setCatModal, openAdd, openEdit,       // menuH
  setConfirmDel,                        // App.jsx local
  setUserModal,                         // App.jsx local (user management)
}) {
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"10px 16px"}}>
        {allCats.filter(k=>k.key!=="semua").map(kat=>{
          const items_=menu.filter(m=>m.kategori===kat.key);
          if(!items_.length) return null;
          return(
            <div key={kat.key} style={{marginBottom:16}}>
              <div style={{fontSize:TYPOGRAPHY.small.fontSize,fontWeight:700,color:G,padding:"3px 8px",background:COLOR_PALETTE.primaryLight,borderRadius:RADIUS.sm,marginBottom:6}}>{kat.label} ({items_.length})</div>
