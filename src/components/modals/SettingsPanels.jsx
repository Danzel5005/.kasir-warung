...
function PasswordInput({ value, onChange, placeholder, style, compact = false }) {
...
export function PricingSettingsTab({ settingsH, menu, cats }) {
  ...
  const lowStockPanel = <div style={{ paddingBottom: 12, borderBottom: `1px solid ${BD}`, marginBottom: 12 }}>
    ...
    <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
      <input type="number" min="1" max="999" value={lowStock} onChange={(event) => setLowStock(event.target.value)} onKeyDown={(event) => event.key === "Enter" && settingsH.setLowStockThreshold(lowStock)} placeholder="5" style={{ ...fieldStyle, width: 90 }} />
