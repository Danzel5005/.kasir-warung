...
export default function LanSyncStatus() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      if (alive) setState({ connected: !!client?.connected, pending: Number(outbox?.pending) || 0 });
    };
    refresh();
    const timer = setInterval(refresh, 5000);
    const off = window.kasirAPI?.onClientEvent?.((event) => {
      if (["connected", "disconnected", "reserve-offline", "reserve-timeout"].includes(event?.kind)) refresh();
    });
    return () => { alive = false; clearInterval(timer); if (typeof off === "function") off(); };
  }, []);
  if (!state.connected && !state.pending) return null;
  const synced = state.connected && !state.pending;
  return <div title={synced ? "Terhubung ke Device A" : "Ada transaksi yang menunggu sinkronisasi"}
    ...
    {synced ? "LAN tersinkron" : `${state.pending} transaksi menunggu sinkron`}
  </div>;
}